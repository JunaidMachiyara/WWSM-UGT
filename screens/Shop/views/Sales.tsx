...
const Sales: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-medium p-8 rounded-lg shadow-lg max-w-5xl mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Record New Invoice</h2>
      {successMessage && (
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select id="customer" value={customerId} onChange={e => setCustomerId(e.target.value)} className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-primary" required>
                    <option value="">Select a customer</option>
                    {shopCustomers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
            </div>
            <div>
                <label htmlFor="saleDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Invoice Date</label>
                <input type="date" id="saleDate" value={saleDate} onChange={e => setSaleDate(e.target.value)} className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-primary" required />
            </div>
        </div>


        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-4">Invoice Items</h3>
          <div className="space-y-4">
            {items.map((item, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end border-t border-gray-200 dark:border-gray-700 pt-4 first:pt-0 first:border-none">
                ...
                <div className="md:col-span-1">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">In Stock</label>
                  <div className="mt-1 p-2 h-[42px] flex items-center">
                    {item.productId ? (
                        <span className="font-semibold text-gray-700 dark:text-gray-200">{item.stock}</span>
                    ) : (
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Sale Price/Unit</label>
                  <input type="number" placeholder="Price" value={item.salePrice} onChange={e => handleItemChange(index, 'salePrice', e.target.value)} className="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-primary" min="0.01" step="0.01" required />
                ...
                <div className="md:col-span-1">
                  <label className="block text-sm font-medium text-transparent dark:text-transparent hidden md:block">&nbsp;</label>
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center text-xl font-bold">
                    <span className="text-gray-800 dark:text-white">Total Invoice Amount:</span>
                    <span className="text-primary dark:text-blue-400">${totalAmount.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                    <label htmlFor="cashPaid" className="text-gray-700 dark:text-gray-300 font-medium">Amount Paid (Cash):</label>
                    <input type="number" id="cashPaid" value={cashPaid} onChange={e => setCashPaid(parseFloat(e.target.value) || 0)} className="w-1/3 text-right border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-primary" min="0" step="0.01" />
                </div>
                <div className="flex justify-between items-center text-lg font-semibold">
                     <span className="text-gray-600 dark:text-gray-400">Amount on Credit:</span>
